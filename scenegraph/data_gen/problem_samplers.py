...
def get_domain_sampler(domain_version):
    ...
    return domain_to_sampler[domain_version]
...
class TaskSamplerV5(TaskSamplerBase):

    def __init__(self, domain, scenegraph, bagslots):
        super(TaskSamplerV5, self).__init__(scenegraph)
    ...
    def generate_pddl_predicates(self):
        ...
        # itemAtLocation, smallItem, mediumItem, largeItem, itemClass
        for o_id in self.objects['all']:
            str_obj_name = self.object_names[o_id]
            str_obj_loc_name = self.location_names[o_id]            
            str_obj_class = self.sg.object[o_id].class_.replace(' ', '')
