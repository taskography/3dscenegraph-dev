
(define (problem taskograph_rearrangement) (:domain taskography_v3)
  (:objects
        agent - agent
	bagslot1 - bagslot
	bagslot2 - bagslot
	bagslot3 - bagslot
	bagslot4 - bagslot
	bagslot5 - bagslot
	item10_bottle_smallitem - item
	item11_bottle_smallitem - item
	item12_bottle_smallitem - item
	item13_bottle_smallitem - item
	item14_cup_smallitem - item
	item15_cup_smallitem - item
	item16_bowl_smallitem - item
	item49_potted_plant_largeitem - item
	item64_tv_largeitem - item
	item65_remote_smallitem - item
	item66_remote_smallitem - item
	item79_vase_mediumitem - item
	item80_teddy_bear_mediumitem - item
	item9_baseball_bat_largeitem - item
	location_Xneg10_Yneg17_place66_room7_floorA - location
	location_Xneg10_Yneg19_place14_room7_floorA - location
	location_Xneg10_Ypos143_place40_room23_floorB - location
	location_Xneg11_Ypos34_place4_room14_floorA - location
	location_Xneg12_Ypos108_place11_room23_floorB - location
	location_Xneg12_Ypos9_place71_room8_floorB - location
	location_Xneg15_Ypos27_place81_room5_floorC - location
	location_Xneg19_Yneg16_place57_room7_floorA - location
	location_Xneg20_Ypos141_place46_room23_floorB - location
	location_Xneg27_Ypos107_place61_room23_floorB - location
	location_Xneg29_Yneg12_place67_room8_floorB - location
	location_Xneg30_Yneg10_place54_room10_floorC - location
	location_Xneg30_Ypos42_place19_room5_floorC - location
	location_Xneg31_Ypos3_place72_room8_floorB - location
	location_Xneg39_Yneg20_place56_room7_floorA - location
	location_Xneg39_Ypos145_place45_room23_floorB - location
	location_Xneg42_Ypos54_place90_room5_floorC - location
	location_Xneg43_Ypos35_place86_room5_floorC - location
	location_Xneg55_Ypos42_place6_room2_floorB - location
	location_Xneg56_Ypos118_place65_room18_floorC - location
	location_Xneg57_Ypos0_place18_room1_floorA - location
	location_Xneg57_Ypos70_place53_room18_floorC - location
	location_Xneg58_Ypos48_place79_room2_floorB - location
	location_Xneg59_Ypos38_place84_room2_floorB - location
	location_Xneg5_Yneg1_place17_room8_floorB - location
	location_Xneg5_Yneg42_place27_room7_floorA - location
	location_Xneg5_Ypos0_place15_room10_floorC - location
	location_Xneg61_Yneg7_place55_room9_floorB - location
	location_Xneg61_Ypos33_place84_room2_floorB - location
	location_Xneg61_Ypos42_place84_room2_floorB - location
	location_Xneg62_Ypos86_place87_room21_floorB - location
	location_Xneg64_Yneg12_place80_room1_floorA - location
	location_Xneg64_Ypos6_place89_room1_floorA - location
	location_Xneg65_Ypos108_place38_room21_floorB - location
	location_Xneg66_Ypos139_place41_room20_floorB - location
	location_Xneg67_Ypos36_place59_room12_floorB - location
	location_Xneg68_Ypos36_place82_room12_floorB - location
	location_Xneg6_Ypos46_place24_room26_floorA - location
	location_Xneg71_Ypos7_place8_room9_floorB - location
	location_Xneg72_Ypos135_place12_room20_floorB - location
	location_Xneg72_Ypos137_place78_room20_floorB - location
	location_Xneg72_Ypos90_place7_room21_floorB - location
	location_Xneg74_Ypos70_place91_room21_floorB - location
	location_Xneg75_Ypos31_place25_room9_floorB - location
	location_Xneg79_Ypos36_place92_room12_floorB - location
	location_Xneg79_Ypos42_place9_room12_floorB - location
	location_Xneg7_Ypos43_place21_room17_floorB - location
	location_Xneg83_Ypos138_place44_room20_floorB - location
	location_Xneg84_Ypos8_place74_room9_floorB - location
	location_Xneg8_Ypos43_place5_room26_floorA - location
	location_Xneg91_Ypos118_place39_room21_floorB - location
	location_Xneg92_Ypos39_place92_room12_floorB - location
	location_Xneg92_Ypos64_place83_room21_floorB - location
	location_Xneg92_Ypos74_place83_room21_floorB - location
	location_Xneg94_Ypos66_place83_room21_floorB - location
	location_Xneg9_Ypos73_place10_room18_floorC - location
	location_Xpos11_Ypos57_place93_room17_floorB - location
	location_Xpos11_Ypos8_place35_room14_floorA - location
	location_Xpos12_Ypos2_place52_room7_floorA - location
	location_Xpos15_Ypos7_place70_room8_floorB - location
	location_Xpos16_Ypos12_place69_room8_floorB - location
	location_Xpos17_Yneg13_place75_room8_floorB - location
	location_Xpos17_Yneg21_place68_room7_floorA - location
	location_Xpos18_Yneg1_place73_room10_floorC - location
	location_Xpos1_Ypos141_place42_room23_floorB - location
	location_Xpos21_Ypos135_place43_room23_floorB - location
	location_Xpos25_Ypos22_place34_room14_floorA - location
	location_Xpos25_Ypos69_place47_room23_floorB - location
	location_Xpos26_Ypos52_place36_room18_floorC - location
	location_Xpos29_Ypos0_place73_room10_floorC - location
	location_Xpos31_Ypos75_place62_room22_floorA - location
	location_Xpos32_Ypos73_place23_room16_floorB - location
	location_Xpos33_Ypos31_place58_room6_floorA - location
	location_Xpos33_Ypos85_place62_room22_floorA - location
	location_Xpos33_Ypos94_place51_room22_floorA - location
	location_Xpos33_Ypos95_place64_room22_floorA - location
	location_Xpos37_Ypos85_place3_room22_floorA - location
	location_Xpos39_Ypos93_place37_room18_floorC - location
	location_Xpos40_Ypos117_place85_room3_floorB - location
	location_Xpos43_Ypos118_place50_room19_floorA - location
	location_Xpos43_Ypos35_place88_room4_floorB - location
	location_Xpos44_Ypos109_place49_room19_floorA - location
	location_Xpos45_Ypos59_place77_room6_floorA - location
	location_Xpos45_Ypos76_place63_room22_floorA - location
	location_Xpos47_Ypos108_place13_room19_floorA - location
	location_Xpos47_Ypos84_place33_room22_floorA - location
	location_Xpos48_Ypos77_place20_room16_floorB - location
	location_Xpos4_Ypos117_place28_room25_floorA - location
	location_Xpos50_Ypos113_place48_room19_floorA - location
	location_Xpos52_Ypos48_place16_room6_floorA - location
	location_Xpos54_Ypos112_place1_room3_floorB - location
	location_Xpos54_Ypos45_place2_room4_floorB - location
	location_Xpos59_Ypos36_place32_room4_floorB - location
	location_Xpos59_Ypos48_place31_room4_floorB - location
	location_Xpos5_Ypos107_place0_room25_floorA - location
	location_Xpos5_Ypos108_place60_room23_floorB - location
	location_Xpos60_Ypos119_place30_room3_floorB - location
	location_Xpos61_Ypos108_place29_room3_floorB - location
	location_Xpos63_Ypos69_place26_room4_floorB - location
	location_Xpos66_Ypos83_place22_room22_floorA - location
	location_Xpos67_Ypos56_place76_room6_floorA - location
	place0_door_room25_playroom - place
	place10_door_room18_corridor - place
	place11_door_room23_living_room - place
	place12_door_room20_dining_room - place
	place13_door_room19_dining_room - place
	place14_door_room7_bedroom - place
	place15_door_room10_bedroom - place
	place16_door_room6_bedroom - place
	place17_door_room8_bedroom - place
	place18_door_room1_bathroom - place
	place19_door_room5_bathroom - place
	place1_door_room3_bathroom - place
	place20_door_room16_corridor - place
	place21_door_room17_corridor - place
	place22_item64_tv - place
	place23_item66_remote - place
	place24_item9_baseball_bat - place
	place25_item14_cup - place
	place26_item15_cup - place
	place27_item80_teddy_bear - place
	place28_receptacle1_bench - place
	place29_receptacle2_bench - place
	place2_door_room4_bathroom - place
	place30_receptacle3_bench - place
	place31_receptacle4_bench - place
	place32_receptacle5_bench - place
	place33_receptacle6_bench - place
	place34_receptacle7_bench - place
	place35_receptacle8_bench - place
	place36_receptacle17_chair - place
	place37_receptacle18_chair - place
	place38_receptacle19_chair - place
	place39_receptacle20_chair - place
	place3_door_room22_living_room - place
	place40_receptacle21_chair - place
	place41_receptacle22_chair - place
	place42_receptacle23_chair - place
	place43_receptacle24_chair - place
	place44_receptacle25_chair - place
	place45_receptacle26_chair - place
	place46_receptacle27_chair - place
	place47_receptacle28_chair - place
	place48_receptacle29_chair - place
	place49_receptacle30_chair - place
	place4_door_room14_corridor - place
	place50_receptacle31_chair - place
	place51_receptacle32_chair - place
	place52_receptacle33_chair - place
	place53_receptacle34_chair - place
	place54_receptacle35_chair - place
	place55_receptacle36_chair - place
	place56_receptacle37_chair - place
	place57_receptacle38_chair - place
	place58_receptacle39_chair - place
	place59_receptacle40_chair - place
	place5_door_room26_staircase - place
	place60_receptacle41_couch - place
	place61_receptacle42_couch - place
	place62_receptacle43_couch - place
	place63_receptacle44_couch - place
	place64_receptacle45_couch - place
	place65_receptacle46_couch - place
	place66_receptacle47_couch - place
	place67_receptacle48_couch - place
	place68_receptacle50_bed - place
	place69_receptacle51_bed - place
	place6_door_room2_bathroom - place
	place70_receptacle52_bed - place
	place71_receptacle53_bed - place
	place72_receptacle54_bed - place
	place73_receptacle55_bed - place
	place74_receptacle56_bed - place
	place75_receptacle57_bed - place
	place76_receptacle58_bed - place
	place77_receptacle59_bed - place
	place78_receptacle60_dining_table - place
	place79_receptacle61_toilet - place
	place7_door_room21_kitchen - place
	place80_receptacle62_toilet - place
	place81_receptacle63_toilet - place
	place82_receptacle67_microwave - place
	place83_receptacle68_oven - place
	place84_receptacle69_sink - place
	place85_receptacle70_sink - place
	place86_receptacle71_sink - place
	place87_receptacle72_sink - place
	place88_receptacle73_sink - place
	place89_receptacle74_sink - place
	place8_door_room9_bedroom - place
	place90_receptacle75_sink - place
	place91_receptacle76_refrigerator - place
	place92_receptacle77_refrigerator - place
	place93_receptacle78_refrigerator - place
	place9_door_room12_closet - place
	receptacle17_chair - receptacle
	receptacle18_chair - receptacle
	receptacle19_chair - receptacle
	receptacle1_bench - receptacle
	receptacle20_chair - receptacle
	receptacle21_chair - receptacle
	receptacle22_chair - receptacle
	receptacle23_chair - receptacle
	receptacle24_chair - receptacle
	receptacle25_chair - receptacle
	receptacle26_chair - receptacle
	receptacle27_chair - receptacle
	receptacle28_chair - receptacle
	receptacle29_chair - receptacle
	receptacle2_bench - receptacle
	receptacle30_chair - receptacle
	receptacle31_chair - receptacle
	receptacle32_chair - receptacle
	receptacle33_chair - receptacle
	receptacle34_chair - receptacle
	receptacle35_chair - receptacle
	receptacle36_chair - receptacle
	receptacle37_chair - receptacle
	receptacle38_chair - receptacle
	receptacle39_chair - receptacle
	receptacle3_bench - receptacle
	receptacle40_chair - receptacle
	receptacle41_couch - receptacle
	receptacle42_couch - receptacle
	receptacle43_couch - receptacle
	receptacle44_couch - receptacle
	receptacle45_couch - receptacle
	receptacle46_couch - receptacle
	receptacle47_couch - receptacle
	receptacle48_couch - receptacle
	receptacle4_bench - receptacle
	receptacle50_bed - receptacle
	receptacle51_bed - receptacle
	receptacle52_bed - receptacle
	receptacle53_bed - receptacle
	receptacle54_bed - receptacle
	receptacle55_bed - receptacle
	receptacle56_bed - receptacle
	receptacle57_bed - receptacle
	receptacle58_bed - receptacle
	receptacle59_bed - receptacle
	receptacle5_bench - receptacle
	receptacle60_dining_table - receptacle
	receptacle61_toilet - receptacle
	receptacle62_toilet - receptacle
	receptacle63_toilet - receptacle
	receptacle67_microwave - receptacle
	receptacle68_oven - receptacle
	receptacle69_sink - receptacle
	receptacle6_bench - receptacle
	receptacle70_sink - receptacle
	receptacle71_sink - receptacle
	receptacle72_sink - receptacle
	receptacle73_sink - receptacle
	receptacle74_sink - receptacle
	receptacle75_sink - receptacle
	receptacle76_refrigerator - receptacle
	receptacle77_refrigerator - receptacle
	receptacle78_refrigerator - receptacle
	receptacle7_bench - receptacle
	receptacle8_bench - receptacle
	room10_bedroom - room
	room12_closet - room
	room14_corridor - room
	room16_corridor - room
	room17_corridor - room
	room18_corridor - room
	room19_dining_room - room
	room1_bathroom - room
	room20_dining_room - room
	room21_kitchen - room
	room22_living_room - room
	room23_living_room - room
	room25_playroom - room
	room26_staircase - room
	room2_bathroom - room
	room3_bathroom - room
	room4_bathroom - room
	room5_bathroom - room
	room6_bedroom - room
	room7_bedroom - room
	room8_bedroom - room
	room9_bedroom - room
  )
  (:init 
	(atlocation agent location_Xneg5_Yneg1_place17_room8_floorB)
	(inanyreceptacle item10_bottle_smallitem)
	(inanyreceptacle item11_bottle_smallitem)
	(inanyreceptacle item12_bottle_smallitem)
	(inanyreceptacle item13_bottle_smallitem)
	(inanyreceptacle item16_bowl_smallitem)
	(inanyreceptacle item49_potted_plant_largeitem)
	(inanyreceptacle item65_remote_smallitem)
	(inanyreceptacle item79_vase_mediumitem)
	(inplace agent place17_door_room8_bedroom)
	(inreceptacle item10_bottle_smallitem receptacle69_sink)
	(inreceptacle item11_bottle_smallitem receptacle68_oven)
	(inreceptacle item12_bottle_smallitem receptacle68_oven)
	(inreceptacle item13_bottle_smallitem receptacle69_sink)
	(inreceptacle item16_bowl_smallitem receptacle77_refrigerator)
	(inreceptacle item49_potted_plant_largeitem receptacle55_bed)
	(inreceptacle item65_remote_smallitem receptacle43_couch)
	(inreceptacle item79_vase_mediumitem receptacle55_bed)
	(inroom agent room8_bedroom)
	(itematlocation item10_bottle_smallitem location_Xneg61_Ypos33_place84_room2_floorB)
	(itematlocation item11_bottle_smallitem location_Xneg92_Ypos64_place83_room21_floorB)
	(itematlocation item12_bottle_smallitem location_Xneg94_Ypos66_place83_room21_floorB)
	(itematlocation item13_bottle_smallitem location_Xneg61_Ypos42_place84_room2_floorB)
	(itematlocation item14_cup_smallitem location_Xneg75_Ypos31_place25_room9_floorB)
	(itematlocation item15_cup_smallitem location_Xpos63_Ypos69_place26_room4_floorB)
	(itematlocation item16_bowl_smallitem location_Xneg92_Ypos39_place92_room12_floorB)
	(itematlocation item49_potted_plant_largeitem location_Xpos29_Ypos0_place73_room10_floorC)
	(itematlocation item64_tv_largeitem location_Xpos66_Ypos83_place22_room22_floorA)
	(itematlocation item65_remote_smallitem location_Xpos31_Ypos75_place62_room22_floorA)
	(itematlocation item66_remote_smallitem location_Xpos32_Ypos73_place23_room16_floorB)
	(itematlocation item79_vase_mediumitem location_Xpos29_Ypos0_place73_room10_floorC)
	(itematlocation item80_teddy_bear_mediumitem location_Xneg5_Yneg42_place27_room7_floorA)
	(itematlocation item9_baseball_bat_largeitem location_Xneg6_Ypos46_place24_room26_floorA)
	(largeitem item49_potted_plant_largeitem)
	(largeitem item64_tv_largeitem)
	(largeitem item9_baseball_bat_largeitem)
	(locationinplace location_Xneg10_Yneg17_place66_room7_floorA place66_receptacle47_couch)
	(locationinplace location_Xneg10_Yneg19_place14_room7_floorA place14_door_room7_bedroom)
	(locationinplace location_Xneg10_Ypos143_place40_room23_floorB place40_receptacle21_chair)
	(locationinplace location_Xneg11_Ypos34_place4_room14_floorA place4_door_room14_corridor)
	(locationinplace location_Xneg12_Ypos108_place11_room23_floorB place11_door_room23_living_room)
	(locationinplace location_Xneg12_Ypos9_place71_room8_floorB place71_receptacle53_bed)
	(locationinplace location_Xneg15_Ypos27_place81_room5_floorC place81_receptacle63_toilet)
	(locationinplace location_Xneg19_Yneg16_place57_room7_floorA place57_receptacle38_chair)
	(locationinplace location_Xneg20_Ypos141_place46_room23_floorB place46_receptacle27_chair)
	(locationinplace location_Xneg27_Ypos107_place61_room23_floorB place61_receptacle42_couch)
	(locationinplace location_Xneg29_Yneg12_place67_room8_floorB place67_receptacle48_couch)
	(locationinplace location_Xneg30_Yneg10_place54_room10_floorC place54_receptacle35_chair)
	(locationinplace location_Xneg30_Ypos42_place19_room5_floorC place19_door_room5_bathroom)
	(locationinplace location_Xneg31_Ypos3_place72_room8_floorB place72_receptacle54_bed)
	(locationinplace location_Xneg39_Yneg20_place56_room7_floorA place56_receptacle37_chair)
	(locationinplace location_Xneg39_Ypos145_place45_room23_floorB place45_receptacle26_chair)
	(locationinplace location_Xneg42_Ypos54_place90_room5_floorC place90_receptacle75_sink)
	(locationinplace location_Xneg43_Ypos35_place86_room5_floorC place86_receptacle71_sink)
	(locationinplace location_Xneg55_Ypos42_place6_room2_floorB place6_door_room2_bathroom)
	(locationinplace location_Xneg56_Ypos118_place65_room18_floorC place65_receptacle46_couch)
	(locationinplace location_Xneg57_Ypos0_place18_room1_floorA place18_door_room1_bathroom)
	(locationinplace location_Xneg57_Ypos70_place53_room18_floorC place53_receptacle34_chair)
	(locationinplace location_Xneg58_Ypos48_place79_room2_floorB place79_receptacle61_toilet)
	(locationinplace location_Xneg59_Ypos38_place84_room2_floorB place84_receptacle69_sink)
	(locationinplace location_Xneg5_Yneg1_place17_room8_floorB place17_door_room8_bedroom)
	(locationinplace location_Xneg5_Yneg42_place27_room7_floorA place27_item80_teddy_bear)
	(locationinplace location_Xneg5_Ypos0_place15_room10_floorC place15_door_room10_bedroom)
	(locationinplace location_Xneg61_Yneg7_place55_room9_floorB place55_receptacle36_chair)
	(locationinplace location_Xneg61_Ypos33_place84_room2_floorB place84_receptacle69_sink)
	(locationinplace location_Xneg61_Ypos42_place84_room2_floorB place84_receptacle69_sink)
	(locationinplace location_Xneg62_Ypos86_place87_room21_floorB place87_receptacle72_sink)
	(locationinplace location_Xneg64_Yneg12_place80_room1_floorA place80_receptacle62_toilet)
	(locationinplace location_Xneg64_Ypos6_place89_room1_floorA place89_receptacle74_sink)
	(locationinplace location_Xneg65_Ypos108_place38_room21_floorB place38_receptacle19_chair)
	(locationinplace location_Xneg66_Ypos139_place41_room20_floorB place41_receptacle22_chair)
	(locationinplace location_Xneg67_Ypos36_place59_room12_floorB place59_receptacle40_chair)
	(locationinplace location_Xneg68_Ypos36_place82_room12_floorB place82_receptacle67_microwave)
	(locationinplace location_Xneg6_Ypos46_place24_room26_floorA place24_item9_baseball_bat)
	(locationinplace location_Xneg71_Ypos7_place8_room9_floorB place8_door_room9_bedroom)
	(locationinplace location_Xneg72_Ypos135_place12_room20_floorB place12_door_room20_dining_room)
	(locationinplace location_Xneg72_Ypos137_place78_room20_floorB place78_receptacle60_dining_table)
	(locationinplace location_Xneg72_Ypos90_place7_room21_floorB place7_door_room21_kitchen)
	(locationinplace location_Xneg74_Ypos70_place91_room21_floorB place91_receptacle76_refrigerator)
	(locationinplace location_Xneg75_Ypos31_place25_room9_floorB place25_item14_cup)
	(locationinplace location_Xneg79_Ypos36_place92_room12_floorB place92_receptacle77_refrigerator)
	(locationinplace location_Xneg79_Ypos42_place9_room12_floorB place9_door_room12_closet)
	(locationinplace location_Xneg7_Ypos43_place21_room17_floorB place21_door_room17_corridor)
	(locationinplace location_Xneg83_Ypos138_place44_room20_floorB place44_receptacle25_chair)
	(locationinplace location_Xneg84_Ypos8_place74_room9_floorB place74_receptacle56_bed)
	(locationinplace location_Xneg8_Ypos43_place5_room26_floorA place5_door_room26_staircase)
	(locationinplace location_Xneg91_Ypos118_place39_room21_floorB place39_receptacle20_chair)
	(locationinplace location_Xneg92_Ypos39_place92_room12_floorB place92_receptacle77_refrigerator)
	(locationinplace location_Xneg92_Ypos64_place83_room21_floorB place83_receptacle68_oven)
	(locationinplace location_Xneg92_Ypos74_place83_room21_floorB place83_receptacle68_oven)
	(locationinplace location_Xneg94_Ypos66_place83_room21_floorB place83_receptacle68_oven)
	(locationinplace location_Xneg9_Ypos73_place10_room18_floorC place10_door_room18_corridor)
	(locationinplace location_Xpos11_Ypos57_place93_room17_floorB place93_receptacle78_refrigerator)
	(locationinplace location_Xpos11_Ypos8_place35_room14_floorA place35_receptacle8_bench)
	(locationinplace location_Xpos12_Ypos2_place52_room7_floorA place52_receptacle33_chair)
	(locationinplace location_Xpos15_Ypos7_place70_room8_floorB place70_receptacle52_bed)
	(locationinplace location_Xpos16_Ypos12_place69_room8_floorB place69_receptacle51_bed)
	(locationinplace location_Xpos17_Yneg13_place75_room8_floorB place75_receptacle57_bed)
	(locationinplace location_Xpos17_Yneg21_place68_room7_floorA place68_receptacle50_bed)
	(locationinplace location_Xpos18_Yneg1_place73_room10_floorC place73_receptacle55_bed)
	(locationinplace location_Xpos1_Ypos141_place42_room23_floorB place42_receptacle23_chair)
	(locationinplace location_Xpos21_Ypos135_place43_room23_floorB place43_receptacle24_chair)
	(locationinplace location_Xpos25_Ypos22_place34_room14_floorA place34_receptacle7_bench)
	(locationinplace location_Xpos25_Ypos69_place47_room23_floorB place47_receptacle28_chair)
	(locationinplace location_Xpos26_Ypos52_place36_room18_floorC place36_receptacle17_chair)
	(locationinplace location_Xpos29_Ypos0_place73_room10_floorC place73_receptacle55_bed)
	(locationinplace location_Xpos31_Ypos75_place62_room22_floorA place62_receptacle43_couch)
	(locationinplace location_Xpos32_Ypos73_place23_room16_floorB place23_item66_remote)
	(locationinplace location_Xpos33_Ypos31_place58_room6_floorA place58_receptacle39_chair)
	(locationinplace location_Xpos33_Ypos85_place62_room22_floorA place62_receptacle43_couch)
	(locationinplace location_Xpos33_Ypos94_place51_room22_floorA place51_receptacle32_chair)
	(locationinplace location_Xpos33_Ypos95_place64_room22_floorA place64_receptacle45_couch)
	(locationinplace location_Xpos37_Ypos85_place3_room22_floorA place3_door_room22_living_room)
	(locationinplace location_Xpos39_Ypos93_place37_room18_floorC place37_receptacle18_chair)
	(locationinplace location_Xpos40_Ypos117_place85_room3_floorB place85_receptacle70_sink)
	(locationinplace location_Xpos43_Ypos118_place50_room19_floorA place50_receptacle31_chair)
	(locationinplace location_Xpos43_Ypos35_place88_room4_floorB place88_receptacle73_sink)
	(locationinplace location_Xpos44_Ypos109_place49_room19_floorA place49_receptacle30_chair)
	(locationinplace location_Xpos45_Ypos59_place77_room6_floorA place77_receptacle59_bed)
	(locationinplace location_Xpos45_Ypos76_place63_room22_floorA place63_receptacle44_couch)
	(locationinplace location_Xpos47_Ypos108_place13_room19_floorA place13_door_room19_dining_room)
	(locationinplace location_Xpos47_Ypos84_place33_room22_floorA place33_receptacle6_bench)
	(locationinplace location_Xpos48_Ypos77_place20_room16_floorB place20_door_room16_corridor)
	(locationinplace location_Xpos4_Ypos117_place28_room25_floorA place28_receptacle1_bench)
	(locationinplace location_Xpos50_Ypos113_place48_room19_floorA place48_receptacle29_chair)
	(locationinplace location_Xpos52_Ypos48_place16_room6_floorA place16_door_room6_bedroom)
	(locationinplace location_Xpos54_Ypos112_place1_room3_floorB place1_door_room3_bathroom)
	(locationinplace location_Xpos54_Ypos45_place2_room4_floorB place2_door_room4_bathroom)
	(locationinplace location_Xpos59_Ypos36_place32_room4_floorB place32_receptacle5_bench)
	(locationinplace location_Xpos59_Ypos48_place31_room4_floorB place31_receptacle4_bench)
	(locationinplace location_Xpos5_Ypos107_place0_room25_floorA place0_door_room25_playroom)
	(locationinplace location_Xpos5_Ypos108_place60_room23_floorB place60_receptacle41_couch)
	(locationinplace location_Xpos60_Ypos119_place30_room3_floorB place30_receptacle3_bench)
	(locationinplace location_Xpos61_Ypos108_place29_room3_floorB place29_receptacle2_bench)
	(locationinplace location_Xpos63_Ypos69_place26_room4_floorB place26_item15_cup)
	(locationinplace location_Xpos66_Ypos83_place22_room22_floorA place22_item64_tv)
	(locationinplace location_Xpos67_Ypos56_place76_room6_floorA place76_receptacle58_bed)
	(mediumitem item79_vase_mediumitem)
	(mediumitem item80_teddy_bear_mediumitem)
	(placeinroom place0_door_room25_playroom room25_playroom)
	(placeinroom place10_door_room18_corridor room18_corridor)
	(placeinroom place11_door_room23_living_room room23_living_room)
	(placeinroom place12_door_room20_dining_room room20_dining_room)
	(placeinroom place13_door_room19_dining_room room19_dining_room)
	(placeinroom place14_door_room7_bedroom room7_bedroom)
	(placeinroom place15_door_room10_bedroom room10_bedroom)
	(placeinroom place16_door_room6_bedroom room6_bedroom)
	(placeinroom place17_door_room8_bedroom room8_bedroom)
	(placeinroom place18_door_room1_bathroom room1_bathroom)
	(placeinroom place19_door_room5_bathroom room5_bathroom)
	(placeinroom place1_door_room3_bathroom room3_bathroom)
	(placeinroom place20_door_room16_corridor room16_corridor)
	(placeinroom place21_door_room17_corridor room17_corridor)
	(placeinroom place22_item64_tv room22_living_room)
	(placeinroom place23_item66_remote room16_corridor)
	(placeinroom place24_item9_baseball_bat room26_staircase)
	(placeinroom place25_item14_cup room9_bedroom)
	(placeinroom place26_item15_cup room4_bathroom)
	(placeinroom place27_item80_teddy_bear room7_bedroom)
	(placeinroom place28_receptacle1_bench room25_playroom)
	(placeinroom place29_receptacle2_bench room3_bathroom)
	(placeinroom place2_door_room4_bathroom room4_bathroom)
	(placeinroom place30_receptacle3_bench room3_bathroom)
	(placeinroom place31_receptacle4_bench room4_bathroom)
	(placeinroom place32_receptacle5_bench room4_bathroom)
	(placeinroom place33_receptacle6_bench room22_living_room)
	(placeinroom place34_receptacle7_bench room14_corridor)
	(placeinroom place35_receptacle8_bench room14_corridor)
	(placeinroom place36_receptacle17_chair room18_corridor)
	(placeinroom place37_receptacle18_chair room18_corridor)
	(placeinroom place38_receptacle19_chair room21_kitchen)
	(placeinroom place39_receptacle20_chair room21_kitchen)
	(placeinroom place3_door_room22_living_room room22_living_room)
	(placeinroom place40_receptacle21_chair room23_living_room)
	(placeinroom place41_receptacle22_chair room20_dining_room)
	(placeinroom place42_receptacle23_chair room23_living_room)
	(placeinroom place43_receptacle24_chair room23_living_room)
	(placeinroom place44_receptacle25_chair room20_dining_room)
	(placeinroom place45_receptacle26_chair room23_living_room)
	(placeinroom place46_receptacle27_chair room23_living_room)
	(placeinroom place47_receptacle28_chair room23_living_room)
	(placeinroom place48_receptacle29_chair room19_dining_room)
	(placeinroom place49_receptacle30_chair room19_dining_room)
	(placeinroom place4_door_room14_corridor room14_corridor)
	(placeinroom place50_receptacle31_chair room19_dining_room)
	(placeinroom place51_receptacle32_chair room22_living_room)
	(placeinroom place52_receptacle33_chair room7_bedroom)
	(placeinroom place53_receptacle34_chair room18_corridor)
	(placeinroom place54_receptacle35_chair room10_bedroom)
	(placeinroom place55_receptacle36_chair room9_bedroom)
	(placeinroom place56_receptacle37_chair room7_bedroom)
	(placeinroom place57_receptacle38_chair room7_bedroom)
	(placeinroom place58_receptacle39_chair room6_bedroom)
	(placeinroom place59_receptacle40_chair room12_closet)
	(placeinroom place5_door_room26_staircase room26_staircase)
	(placeinroom place60_receptacle41_couch room23_living_room)
	(placeinroom place61_receptacle42_couch room23_living_room)
	(placeinroom place62_receptacle43_couch room22_living_room)
	(placeinroom place63_receptacle44_couch room22_living_room)
	(placeinroom place64_receptacle45_couch room22_living_room)
	(placeinroom place65_receptacle46_couch room18_corridor)
	(placeinroom place66_receptacle47_couch room7_bedroom)
	(placeinroom place67_receptacle48_couch room8_bedroom)
	(placeinroom place68_receptacle50_bed room7_bedroom)
	(placeinroom place69_receptacle51_bed room8_bedroom)
	(placeinroom place6_door_room2_bathroom room2_bathroom)
	(placeinroom place70_receptacle52_bed room8_bedroom)
	(placeinroom place71_receptacle53_bed room8_bedroom)
	(placeinroom place72_receptacle54_bed room8_bedroom)
	(placeinroom place73_receptacle55_bed room10_bedroom)
	(placeinroom place74_receptacle56_bed room9_bedroom)
	(placeinroom place75_receptacle57_bed room8_bedroom)
	(placeinroom place76_receptacle58_bed room6_bedroom)
	(placeinroom place77_receptacle59_bed room6_bedroom)
	(placeinroom place78_receptacle60_dining_table room20_dining_room)
	(placeinroom place79_receptacle61_toilet room2_bathroom)
	(placeinroom place7_door_room21_kitchen room21_kitchen)
	(placeinroom place80_receptacle62_toilet room1_bathroom)
	(placeinroom place81_receptacle63_toilet room5_bathroom)
	(placeinroom place82_receptacle67_microwave room12_closet)
	(placeinroom place83_receptacle68_oven room21_kitchen)
	(placeinroom place84_receptacle69_sink room2_bathroom)
	(placeinroom place85_receptacle70_sink room3_bathroom)
	(placeinroom place86_receptacle71_sink room5_bathroom)
	(placeinroom place87_receptacle72_sink room21_kitchen)
	(placeinroom place88_receptacle73_sink room4_bathroom)
	(placeinroom place89_receptacle74_sink room1_bathroom)
	(placeinroom place8_door_room9_bedroom room9_bedroom)
	(placeinroom place90_receptacle75_sink room5_bathroom)
	(placeinroom place91_receptacle76_refrigerator room21_kitchen)
	(placeinroom place92_receptacle77_refrigerator room12_closet)
	(placeinroom place93_receptacle78_refrigerator room17_corridor)
	(placeinroom place9_door_room12_closet room12_closet)
	(placelocation location_Xneg10_Yneg17_place66_room7_floorA place66_receptacle47_couch)
	(placelocation location_Xneg10_Yneg19_place14_room7_floorA place14_door_room7_bedroom)
	(placelocation location_Xneg10_Ypos143_place40_room23_floorB place40_receptacle21_chair)
	(placelocation location_Xneg11_Ypos34_place4_room14_floorA place4_door_room14_corridor)
	(placelocation location_Xneg12_Ypos108_place11_room23_floorB place11_door_room23_living_room)
	(placelocation location_Xneg12_Ypos9_place71_room8_floorB place71_receptacle53_bed)
	(placelocation location_Xneg15_Ypos27_place81_room5_floorC place81_receptacle63_toilet)
	(placelocation location_Xneg19_Yneg16_place57_room7_floorA place57_receptacle38_chair)
	(placelocation location_Xneg20_Ypos141_place46_room23_floorB place46_receptacle27_chair)
	(placelocation location_Xneg27_Ypos107_place61_room23_floorB place61_receptacle42_couch)
	(placelocation location_Xneg29_Yneg12_place67_room8_floorB place67_receptacle48_couch)
	(placelocation location_Xneg30_Yneg10_place54_room10_floorC place54_receptacle35_chair)
	(placelocation location_Xneg30_Ypos42_place19_room5_floorC place19_door_room5_bathroom)
	(placelocation location_Xneg31_Ypos3_place72_room8_floorB place72_receptacle54_bed)
	(placelocation location_Xneg39_Yneg20_place56_room7_floorA place56_receptacle37_chair)
	(placelocation location_Xneg39_Ypos145_place45_room23_floorB place45_receptacle26_chair)
	(placelocation location_Xneg42_Ypos54_place90_room5_floorC place90_receptacle75_sink)
	(placelocation location_Xneg43_Ypos35_place86_room5_floorC place86_receptacle71_sink)
	(placelocation location_Xneg55_Ypos42_place6_room2_floorB place6_door_room2_bathroom)
	(placelocation location_Xneg56_Ypos118_place65_room18_floorC place65_receptacle46_couch)
	(placelocation location_Xneg57_Ypos0_place18_room1_floorA place18_door_room1_bathroom)
	(placelocation location_Xneg57_Ypos70_place53_room18_floorC place53_receptacle34_chair)
	(placelocation location_Xneg58_Ypos48_place79_room2_floorB place79_receptacle61_toilet)
	(placelocation location_Xneg59_Ypos38_place84_room2_floorB place84_receptacle69_sink)
	(placelocation location_Xneg5_Yneg1_place17_room8_floorB place17_door_room8_bedroom)
	(placelocation location_Xneg5_Yneg42_place27_room7_floorA place27_item80_teddy_bear)
	(placelocation location_Xneg5_Ypos0_place15_room10_floorC place15_door_room10_bedroom)
	(placelocation location_Xneg61_Yneg7_place55_room9_floorB place55_receptacle36_chair)
	(placelocation location_Xneg62_Ypos86_place87_room21_floorB place87_receptacle72_sink)
	(placelocation location_Xneg64_Yneg12_place80_room1_floorA place80_receptacle62_toilet)
	(placelocation location_Xneg64_Ypos6_place89_room1_floorA place89_receptacle74_sink)
	(placelocation location_Xneg65_Ypos108_place38_room21_floorB place38_receptacle19_chair)
	(placelocation location_Xneg66_Ypos139_place41_room20_floorB place41_receptacle22_chair)
	(placelocation location_Xneg67_Ypos36_place59_room12_floorB place59_receptacle40_chair)
	(placelocation location_Xneg68_Ypos36_place82_room12_floorB place82_receptacle67_microwave)
	(placelocation location_Xneg6_Ypos46_place24_room26_floorA place24_item9_baseball_bat)
	(placelocation location_Xneg71_Ypos7_place8_room9_floorB place8_door_room9_bedroom)
	(placelocation location_Xneg72_Ypos135_place12_room20_floorB place12_door_room20_dining_room)
	(placelocation location_Xneg72_Ypos137_place78_room20_floorB place78_receptacle60_dining_table)
	(placelocation location_Xneg72_Ypos90_place7_room21_floorB place7_door_room21_kitchen)
	(placelocation location_Xneg74_Ypos70_place91_room21_floorB place91_receptacle76_refrigerator)
	(placelocation location_Xneg75_Ypos31_place25_room9_floorB place25_item14_cup)
	(placelocation location_Xneg79_Ypos36_place92_room12_floorB place92_receptacle77_refrigerator)
	(placelocation location_Xneg79_Ypos42_place9_room12_floorB place9_door_room12_closet)
	(placelocation location_Xneg7_Ypos43_place21_room17_floorB place21_door_room17_corridor)
	(placelocation location_Xneg83_Ypos138_place44_room20_floorB place44_receptacle25_chair)
	(placelocation location_Xneg84_Ypos8_place74_room9_floorB place74_receptacle56_bed)
	(placelocation location_Xneg8_Ypos43_place5_room26_floorA place5_door_room26_staircase)
	(placelocation location_Xneg91_Ypos118_place39_room21_floorB place39_receptacle20_chair)
	(placelocation location_Xneg92_Ypos74_place83_room21_floorB place83_receptacle68_oven)
	(placelocation location_Xneg9_Ypos73_place10_room18_floorC place10_door_room18_corridor)
	(placelocation location_Xpos11_Ypos57_place93_room17_floorB place93_receptacle78_refrigerator)
	(placelocation location_Xpos11_Ypos8_place35_room14_floorA place35_receptacle8_bench)
	(placelocation location_Xpos12_Ypos2_place52_room7_floorA place52_receptacle33_chair)
	(placelocation location_Xpos15_Ypos7_place70_room8_floorB place70_receptacle52_bed)
	(placelocation location_Xpos16_Ypos12_place69_room8_floorB place69_receptacle51_bed)
	(placelocation location_Xpos17_Yneg13_place75_room8_floorB place75_receptacle57_bed)
	(placelocation location_Xpos17_Yneg21_place68_room7_floorA place68_receptacle50_bed)
	(placelocation location_Xpos18_Yneg1_place73_room10_floorC place73_receptacle55_bed)
	(placelocation location_Xpos1_Ypos141_place42_room23_floorB place42_receptacle23_chair)
	(placelocation location_Xpos21_Ypos135_place43_room23_floorB place43_receptacle24_chair)
	(placelocation location_Xpos25_Ypos22_place34_room14_floorA place34_receptacle7_bench)
	(placelocation location_Xpos25_Ypos69_place47_room23_floorB place47_receptacle28_chair)
	(placelocation location_Xpos26_Ypos52_place36_room18_floorC place36_receptacle17_chair)
	(placelocation location_Xpos32_Ypos73_place23_room16_floorB place23_item66_remote)
	(placelocation location_Xpos33_Ypos31_place58_room6_floorA place58_receptacle39_chair)
	(placelocation location_Xpos33_Ypos85_place62_room22_floorA place62_receptacle43_couch)
	(placelocation location_Xpos33_Ypos94_place51_room22_floorA place51_receptacle32_chair)
	(placelocation location_Xpos33_Ypos95_place64_room22_floorA place64_receptacle45_couch)
	(placelocation location_Xpos37_Ypos85_place3_room22_floorA place3_door_room22_living_room)
	(placelocation location_Xpos39_Ypos93_place37_room18_floorC place37_receptacle18_chair)
	(placelocation location_Xpos40_Ypos117_place85_room3_floorB place85_receptacle70_sink)
	(placelocation location_Xpos43_Ypos118_place50_room19_floorA place50_receptacle31_chair)
	(placelocation location_Xpos43_Ypos35_place88_room4_floorB place88_receptacle73_sink)
	(placelocation location_Xpos44_Ypos109_place49_room19_floorA place49_receptacle30_chair)
	(placelocation location_Xpos45_Ypos59_place77_room6_floorA place77_receptacle59_bed)
	(placelocation location_Xpos45_Ypos76_place63_room22_floorA place63_receptacle44_couch)
	(placelocation location_Xpos47_Ypos108_place13_room19_floorA place13_door_room19_dining_room)
	(placelocation location_Xpos47_Ypos84_place33_room22_floorA place33_receptacle6_bench)
	(placelocation location_Xpos48_Ypos77_place20_room16_floorB place20_door_room16_corridor)
	(placelocation location_Xpos4_Ypos117_place28_room25_floorA place28_receptacle1_bench)
	(placelocation location_Xpos50_Ypos113_place48_room19_floorA place48_receptacle29_chair)
	(placelocation location_Xpos52_Ypos48_place16_room6_floorA place16_door_room6_bedroom)
	(placelocation location_Xpos54_Ypos112_place1_room3_floorB place1_door_room3_bathroom)
	(placelocation location_Xpos54_Ypos45_place2_room4_floorB place2_door_room4_bathroom)
	(placelocation location_Xpos59_Ypos36_place32_room4_floorB place32_receptacle5_bench)
	(placelocation location_Xpos59_Ypos48_place31_room4_floorB place31_receptacle4_bench)
	(placelocation location_Xpos5_Ypos107_place0_room25_floorA place0_door_room25_playroom)
	(placelocation location_Xpos5_Ypos108_place60_room23_floorB place60_receptacle41_couch)
	(placelocation location_Xpos60_Ypos119_place30_room3_floorB place30_receptacle3_bench)
	(placelocation location_Xpos61_Ypos108_place29_room3_floorB place29_receptacle2_bench)
	(placelocation location_Xpos63_Ypos69_place26_room4_floorB place26_item15_cup)
	(placelocation location_Xpos66_Ypos83_place22_room22_floorA place22_item64_tv)
	(placelocation location_Xpos67_Ypos56_place76_room6_floorA place76_receptacle58_bed)
	(receptacleatlocation receptacle17_chair location_Xpos26_Ypos52_place36_room18_floorC)
	(receptacleatlocation receptacle18_chair location_Xpos39_Ypos93_place37_room18_floorC)
	(receptacleatlocation receptacle19_chair location_Xneg65_Ypos108_place38_room21_floorB)
	(receptacleatlocation receptacle1_bench location_Xpos4_Ypos117_place28_room25_floorA)
	(receptacleatlocation receptacle20_chair location_Xneg91_Ypos118_place39_room21_floorB)
	(receptacleatlocation receptacle21_chair location_Xneg10_Ypos143_place40_room23_floorB)
	(receptacleatlocation receptacle22_chair location_Xneg66_Ypos139_place41_room20_floorB)
	(receptacleatlocation receptacle23_chair location_Xpos1_Ypos141_place42_room23_floorB)
	(receptacleatlocation receptacle24_chair location_Xpos21_Ypos135_place43_room23_floorB)
	(receptacleatlocation receptacle25_chair location_Xneg83_Ypos138_place44_room20_floorB)
	(receptacleatlocation receptacle26_chair location_Xneg39_Ypos145_place45_room23_floorB)
	(receptacleatlocation receptacle27_chair location_Xneg20_Ypos141_place46_room23_floorB)
	(receptacleatlocation receptacle28_chair location_Xpos25_Ypos69_place47_room23_floorB)
	(receptacleatlocation receptacle29_chair location_Xpos50_Ypos113_place48_room19_floorA)
	(receptacleatlocation receptacle2_bench location_Xpos61_Ypos108_place29_room3_floorB)
	(receptacleatlocation receptacle30_chair location_Xpos44_Ypos109_place49_room19_floorA)
	(receptacleatlocation receptacle31_chair location_Xpos43_Ypos118_place50_room19_floorA)
	(receptacleatlocation receptacle32_chair location_Xpos33_Ypos94_place51_room22_floorA)
	(receptacleatlocation receptacle33_chair location_Xpos12_Ypos2_place52_room7_floorA)
	(receptacleatlocation receptacle34_chair location_Xneg57_Ypos70_place53_room18_floorC)
	(receptacleatlocation receptacle35_chair location_Xneg30_Yneg10_place54_room10_floorC)
	(receptacleatlocation receptacle36_chair location_Xneg61_Yneg7_place55_room9_floorB)
	(receptacleatlocation receptacle37_chair location_Xneg39_Yneg20_place56_room7_floorA)
	(receptacleatlocation receptacle38_chair location_Xneg19_Yneg16_place57_room7_floorA)
	(receptacleatlocation receptacle39_chair location_Xpos33_Ypos31_place58_room6_floorA)
	(receptacleatlocation receptacle3_bench location_Xpos60_Ypos119_place30_room3_floorB)
	(receptacleatlocation receptacle40_chair location_Xneg67_Ypos36_place59_room12_floorB)
	(receptacleatlocation receptacle41_couch location_Xpos5_Ypos108_place60_room23_floorB)
	(receptacleatlocation receptacle42_couch location_Xneg27_Ypos107_place61_room23_floorB)
	(receptacleatlocation receptacle43_couch location_Xpos33_Ypos85_place62_room22_floorA)
	(receptacleatlocation receptacle44_couch location_Xpos45_Ypos76_place63_room22_floorA)
	(receptacleatlocation receptacle45_couch location_Xpos33_Ypos95_place64_room22_floorA)
	(receptacleatlocation receptacle46_couch location_Xneg56_Ypos118_place65_room18_floorC)
	(receptacleatlocation receptacle47_couch location_Xneg10_Yneg17_place66_room7_floorA)
	(receptacleatlocation receptacle48_couch location_Xneg29_Yneg12_place67_room8_floorB)
	(receptacleatlocation receptacle4_bench location_Xpos59_Ypos48_place31_room4_floorB)
	(receptacleatlocation receptacle50_bed location_Xpos17_Yneg21_place68_room7_floorA)
	(receptacleatlocation receptacle51_bed location_Xpos16_Ypos12_place69_room8_floorB)
	(receptacleatlocation receptacle52_bed location_Xpos15_Ypos7_place70_room8_floorB)
	(receptacleatlocation receptacle53_bed location_Xneg12_Ypos9_place71_room8_floorB)
	(receptacleatlocation receptacle54_bed location_Xneg31_Ypos3_place72_room8_floorB)
	(receptacleatlocation receptacle55_bed location_Xpos18_Yneg1_place73_room10_floorC)
	(receptacleatlocation receptacle56_bed location_Xneg84_Ypos8_place74_room9_floorB)
	(receptacleatlocation receptacle57_bed location_Xpos17_Yneg13_place75_room8_floorB)
	(receptacleatlocation receptacle58_bed location_Xpos67_Ypos56_place76_room6_floorA)
	(receptacleatlocation receptacle59_bed location_Xpos45_Ypos59_place77_room6_floorA)
	(receptacleatlocation receptacle5_bench location_Xpos59_Ypos36_place32_room4_floorB)
	(receptacleatlocation receptacle60_dining_table location_Xneg72_Ypos137_place78_room20_floorB)
	(receptacleatlocation receptacle61_toilet location_Xneg58_Ypos48_place79_room2_floorB)
	(receptacleatlocation receptacle62_toilet location_Xneg64_Yneg12_place80_room1_floorA)
	(receptacleatlocation receptacle63_toilet location_Xneg15_Ypos27_place81_room5_floorC)
	(receptacleatlocation receptacle67_microwave location_Xneg68_Ypos36_place82_room12_floorB)
	(receptacleatlocation receptacle68_oven location_Xneg92_Ypos74_place83_room21_floorB)
	(receptacleatlocation receptacle69_sink location_Xneg59_Ypos38_place84_room2_floorB)
	(receptacleatlocation receptacle6_bench location_Xpos47_Ypos84_place33_room22_floorA)
	(receptacleatlocation receptacle70_sink location_Xpos40_Ypos117_place85_room3_floorB)
	(receptacleatlocation receptacle71_sink location_Xneg43_Ypos35_place86_room5_floorC)
	(receptacleatlocation receptacle72_sink location_Xneg62_Ypos86_place87_room21_floorB)
	(receptacleatlocation receptacle73_sink location_Xpos43_Ypos35_place88_room4_floorB)
	(receptacleatlocation receptacle74_sink location_Xneg64_Ypos6_place89_room1_floorA)
	(receptacleatlocation receptacle75_sink location_Xneg42_Ypos54_place90_room5_floorC)
	(receptacleatlocation receptacle76_refrigerator location_Xneg74_Ypos70_place91_room21_floorB)
	(receptacleatlocation receptacle77_refrigerator location_Xneg79_Ypos36_place92_room12_floorB)
	(receptacleatlocation receptacle78_refrigerator location_Xpos11_Ypos57_place93_room17_floorB)
	(receptacleatlocation receptacle7_bench location_Xpos25_Ypos22_place34_room14_floorA)
	(receptacleatlocation receptacle8_bench location_Xpos11_Ypos8_place35_room14_floorA)
	(receptacleopeningtype receptacle67_microwave)
	(receptacleopeningtype receptacle68_oven)
	(receptacleopeningtype receptacle76_refrigerator)
	(receptacleopeningtype receptacle77_refrigerator)
	(receptacleopeningtype receptacle78_refrigerator)
	(roomplace place0_door_room25_playroom room25_playroom)
	(roomplace place10_door_room18_corridor room18_corridor)
	(roomplace place11_door_room23_living_room room23_living_room)
	(roomplace place12_door_room20_dining_room room20_dining_room)
	(roomplace place13_door_room19_dining_room room19_dining_room)
	(roomplace place14_door_room7_bedroom room7_bedroom)
	(roomplace place15_door_room10_bedroom room10_bedroom)
	(roomplace place16_door_room6_bedroom room6_bedroom)
	(roomplace place17_door_room8_bedroom room8_bedroom)
	(roomplace place18_door_room1_bathroom room1_bathroom)
	(roomplace place19_door_room5_bathroom room5_bathroom)
	(roomplace place1_door_room3_bathroom room3_bathroom)
	(roomplace place20_door_room16_corridor room16_corridor)
	(roomplace place21_door_room17_corridor room17_corridor)
	(roomplace place2_door_room4_bathroom room4_bathroom)
	(roomplace place3_door_room22_living_room room22_living_room)
	(roomplace place4_door_room14_corridor room14_corridor)
	(roomplace place5_door_room26_staircase room26_staircase)
	(roomplace place6_door_room2_bathroom room2_bathroom)
	(roomplace place7_door_room21_kitchen room21_kitchen)
	(roomplace place8_door_room9_bedroom room9_bedroom)
	(roomplace place9_door_room12_closet room12_closet)
	(smallitem item10_bottle_smallitem)
	(smallitem item11_bottle_smallitem)
	(smallitem item12_bottle_smallitem)
	(smallitem item13_bottle_smallitem)
	(smallitem item14_cup_smallitem)
	(smallitem item15_cup_smallitem)
	(smallitem item16_bowl_smallitem)
	(smallitem item65_remote_smallitem)
	(smallitem item66_remote_smallitem)
  )
  (:goal (and
	(inreceptacle item80_teddy_bear_mediumitem receptacle76_refrigerator)
	(inreceptacle item13_bottle_smallitem receptacle30_chair)
	(inreceptacle item10_bottle_smallitem receptacle63_toilet)
	(inreceptacle item9_baseball_bat_largeitem receptacle20_chair)
	(inreceptacle item11_bottle_smallitem receptacle43_couch)
	(inreceptacle item65_remote_smallitem receptacle46_couch)
	(inreceptacle item12_bottle_smallitem receptacle3_bench)
	(inreceptacle item64_tv_largeitem receptacle30_chair)
	(inreceptacle item79_vase_mediumitem receptacle52_bed)
	(inreceptacle item16_bowl_smallitem receptacle52_bed)))
)
